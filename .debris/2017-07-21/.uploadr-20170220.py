...
#        if (os.path.exists(dirpath + "/" + filename + "." + ext)):
            return True
        else:
            return False
...
#        infilename = infilename.encode('utf-8','ignore')
...
#        print type(infilename)
        ...
        filename = filename.split(".")[0]
        ext = fileExt.lower()
#        ext = ext.encode('utf-8','ignore')

#        print "ext", str(ext)
#        print("dirpath", dirpath)
#        print(infilename)
#        print(dirpath + "/" + filename + "." + ext)
#        print(os.path.exists(infilename))
#        print("ext", ext)
...
#        print "filenames", filenames
        if args.processes:
            if len(filenamesraw) > 0:
                ...
                filenames = filenamesraw[0:int(args.max_uploadfiles)]
#                self.ext = ext
                self.pool = ThreadPool(processes=int(args.processes))
    ...
    #Gör smartare
                        if (RAW_SKIP_CREATE_ORIGINAL):
                            command = RAW_TOOL_PATH + "exiftool -overwrite_original -tagsfromfile '" + dirpath + "/" + f + "' -r -all:all -ext JPG '" + dirpath + "/" + filename + ".JPG'"
                            print ("Skip create "+ dirpath + "/" + filename + ".JPG_original")
    ...
    #                                print(command)
                        ...
                        set_max = set_max + 1
                        if args.max_uploadfiles:
                            if (set_max >= int(args.max_uploadfiles)):
                                return
...
#            files = self.photo_movie
#            print "Files2", files
#            print "photo_movie2", sorted(self.photo_movie)
